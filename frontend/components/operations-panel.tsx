...
export function OperationsPanel({ notifications }: { notifications: NotificationItem[] }) {
  return (
    <section className="rounded-md border border-[var(--line)] bg-white/82 p-4 shadow-soft">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-md border border-[var(--line)] bg-panel p-3">
        ...
        <div className="rounded-md border border-[var(--line)] bg-panel p-3">
          ...
          <p className="mt-1 text-xs text-[var(--muted)]">{notifications.length} recent</p>
        </div>
        <div className="rounded-md border border-[var(--line)] bg-panel p-3">
      ...
      <div className="mt-4 space-y-3">
        {notifications.map((notification) => (
          <article key={notification.id} className="rounded-md border border-[var(--line)] bg-panel p-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold">{notification.title}</h3>
                <p className="mt-1 text-xs leading-5 text-[var(--muted)]">{notification.body}</p>
              </div>
              <span className="rounded border border-[var(--line)] bg-white px-2 py-1 text-xs">
                {notification.status}
              </span>
            </div>
          </article>
